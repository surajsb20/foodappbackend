                    ...
                    <div class="heading-elements">
                        <ul class="list-inline mb-0">
                            ...
                            <li><a href="{{ route('admin.shops.create') }}"
                                   class="btn btn-primary add-btn btn-darken-3">@lang('shop.index.add_shop')</a></li>
                        </ul>
                    </div>
                </div>
                <div class="card-body collapse in">
                    <div class="card-block card-dashboard table-responsive">
                        <table class="table table-striped table-bordered file-export">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(Setting::get('PRODUCT_ADDONS')==1)
                                            <a href="{{ route('admin.addons.index') }}?shop={{$User->id}}"
                                               class="btn btn-primary btn-darken-3 tab-order"> Addons List</a>
                                        @endif
                                        <a href="{{ route('admin.categories.index') }}?shop={{$User->id}}"
                                           class="btn btn-primary btn-darken-3 tab-order"> Category List</a>

                                        <a href="{{ route('admin.products.index') }}?shop={{$User->id}}"
                                           class="btn btn-primary btn-darken-3 tab-order"> Menu List</a>

                                        @if(Setting::get('DEMO_MODE')==0)
                                            <a class="table-btn btn btn-icon btn-success"
                                               href="{{ route('admin.shops.edit', $User->id) }}"><i
                                        ...
                                        <form id="resource-delete-{{ $User->id }}"
                                              action="{{ route('admin.shops.destroy', $User->id)}}" method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                        </form>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="50">@lang('shop.index.no_record_found')</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="row m-0">
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr>
                                    <th scope="row">
                                        <div class="bg-img order-img"
                                             style="background-image: url(../assets/img/product-1.jpg);"></div>
                                    </th>
                                    <td>Burger Bistro</td>
                                    <td>$100</td>
                                </tr>
